 ...
 * - Structured component layouts: Hero Title, Executive Content, Two-Column, Native Charts
...
function renderContentSlide(pptx, slideData, palette, index, total) {
  ...
  // Executive Summary Card (if content exists)
 ...
 * 3. Two-Column Slide
 */
function renderTwoColumnSlide(pptx, slideData, palette, index, total) {
  const slide = pptx.addSlide();
  slide.background = { color: palette.bg };
  ...
  const leftFormatted = leftCol.map((b) => ({ text: `• ${b.replace(/^[-•*]\s*/, '')}\n\n`, options: { fontSize: 11, color: palette.textDark } }));
  slide.addText(leftFormatted.length > 0 ? leftFormatted : [{ text: 'Key Findings & Drivers', options: { fontSize: 12, color: palette.textMuted } }], {
    x: 1.1,
    y: colY + 0.3,
    w: colW - 0.6,
    h: colH - 0.6,
  ...
  const rightFormatted = rightCol.map((b) => ({ text: `• ${b.replace(/^[-•*]\s*/, '')}\n\n`, options: { fontSize: 11, color: palette.textDark } }));
  slide.addText(rightFormatted.length > 0 ? rightFormatted : [{ text: 'Strategic Implications', options: { fontSize: 12, color: palette.textMuted } }], {
    x: 7.15,
    y: colY + 0.3,
    w: colW - 0.6,
    h: colH - 0.6,
...
/**
 * Generates a full .pptx binary buffer from presentation data.
 * @param {object} presentation - MongoDB presentation document
  ...
  slides.forEach((slide, index) => {
    ...
    switch (layout) {
      case 'title':
        ...
        break;
      case 'two-column':
        renderTwoColumnSlide(pptx, slide, palette, index, total);
        break;
      case 'chart':
